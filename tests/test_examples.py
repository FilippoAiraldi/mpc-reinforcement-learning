...
class TestExamples(unittest.TestCase):
    def test_q_learning(self):
        ...
        np.testing.assert_allclose(J, d["q_learning_J"])
        np.testing.assert_allclose(X, d["q_learning_X"])
        np.testing.assert_allclose(U, d["q_learning_U"])
        np.testing.assert_allclose(R, d["q_learning_R"])
        np.testing.assert_allclose(TD, d["q_learning_TD"])
        pars_expected = d["q_learning_pars"].item()
        for i, par in enumerate(pars.values()):
            np.testing.assert_allclose(par, pars_expected[i])
