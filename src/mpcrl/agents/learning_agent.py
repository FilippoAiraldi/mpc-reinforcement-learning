...
class LearningAgent(
    ...
    def train(
        ...
        ----------
        env : Env[ObsType, ActType]
            A gym environment where to train the agent in (can be a `VectorEnv`).
        episodes : int
        ...
        ------
        ValueError
            Raises if a `VectorEnv` is passed but a multistart mpc is not used.
        ...
        if hasattr(env, "num_envs"):
            ...
            if not self._V.is_multi or num_envs != self._V.starts:
                raise ValueError(
                    "Cannot train on a `VectorEnv` without a multistart mpc algorithm."
                )
        else:
            num_envs = 1

        self._raises = raises
        returns = np.zeros((num_envs, episodes), float)
        seed_generator = generate_seeds(seed, num_envs)
        self.on_training_start(env)

        for episode, current_seed in zip(range(episodes), seed_generator):
    ...
    def train_one_episode(
        ...
        float or array of floats
            The cumulative rewards for this training episode (or array if vectorized).
