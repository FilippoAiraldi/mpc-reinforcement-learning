...
num_envs = 10  # number of envs to run in parallel
...
class LtiSystem(gym.Env[npt.NDArray[np.floating], float]):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.observation_space = gym.spaces.Box(-np.inf, np.inf, shape=(self.nx, 1))
        self.action_space = gym.spaces.Box(  # type: ignore[assignment]
            *self.a_bnd, shape=(self.nu, 1)
        )
...
class LinearMpc(Mpc[cs.SX]):
    ...
    def __init__(self) -> None:
        ...
        nlp = multistart.ParallelMultistartNlp[cs.SX](starts=num_envs)
...
env_fnc = lambda i: lambda: TimeLimit(LtiSystem(), max_episode_steps=int(5 + i))
env = gym.vector.SyncVectorEnv(env_fnc(i) for i in range(mpc.starts))

MonitorEpisodes
# env = MonitorEpisodes(TimeLimit(LtiSystem(), max_episode_steps=int(5)))
...
agent.train(env=env, episodes=2, seed=69)
