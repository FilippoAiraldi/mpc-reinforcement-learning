...
This example takes inspiration from the linear MPC numerical experiment in
:cite:`gros_datadriven_2020`, but applies DPG to it instead of Q-learning. We are
given an RL environment whose cost function is
...
.. math::
   \begin{aligned}
      \min_{x_{0:N}, u_{0:N-1}, \sigma_{1:N}} \quad &
        V_0^\top x_0 + x_N^\top S x_N + \sum_{i=1}^{N}{ w^\top \sigma_i } \\
...
we will employ a first-order LSTD DPG algorithm.
"""
